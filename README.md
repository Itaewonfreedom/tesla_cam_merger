...
GUI는 **다크 테마** 기반이며, 합성 전에 **원본 클립을 앵글별로 미리 재생**해 어떤 이벤트를
내보낼지 확인할 수 있습니다. 메인 화면에서는 **레이아웃만** 고르고, 인코딩·해상도·배속 등
세부 옵션은 **⚙ Export Settings 창**에서 따로 설정합니다.
...
비디오 영역은 **16:9 레터박스**로 고정되어 창 크기와 무관하게 플레이어 비율이 유지됩니다.

**단축키 / 조작**
- `Space` — 재생 / 일시정지
- `Esc` — 프로그램 종료
- 탐색 슬라이더 **드래그** — 끄는 동안 해당 프레임이 실시간으로 표시되고,
  **재생 중에 끌었다면 놓는 순간 자동으로 이어서 재생**됩니다.
  (이 백엔드는 seek가 비동기로 일시정지를 유발하므로, 재생 의도를 따로 두고 seek 안착 후
  `play()`를 다시 거는 방식. 또한 seek가 일으키는 mediaStatus 변화에 첫-프레임 로직이
  오작동하지 않도록 "새 소스 로드 시"에만 동작하게 가드함)
- 🚀 헤더의 로켓 버튼 또는 아무 데서나 **`DOGE`/`ELON`/`MARS`** 를 타이핑하면… 🐕🚀
